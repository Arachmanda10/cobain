...
<main>
    ...
    <div class="form">
        <form action="add_progress.php?id_rapat=<?php echo htmlspecialchars($_GET['id_rapat']); ?>" method="post">
            <label for="action_step">Action Step</label>
            <input type="text" id="action_step" name="action_step" required>
            ...
            <input type="submit" value="Tambah Progress">
        </form>
    </div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Mengambil id_rapat dari URL
    $id_rapat = $_GET['id_rapat'];

    // Query untuk menambah data progress
    $sql = "INSERT INTO progres (id_rapat, action_step, pic) VALUES (?, ?, ?)";

    // Menyiapkan statement
    if ($stmt = $conn->prepare($sql)) {
        // Mengikat parameter
        $stmt->bind_param("iss", $id_rapat, $action_step, $pic);

        // Menjalankan statement
        if ($stmt->execute()) {
            echo "<div class='success-message'>Progress berhasil ditambahkan.</div>";
        } else {
            echo "<div class='error-message'>Gagal menambahkan progress.</div>";
        }
